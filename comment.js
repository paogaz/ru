...
// const START_TIMER_COUNTER = 1; // 0.2 секунды Начальное значение интервала для счетчика (счетчик убывает со случайным интервалом между начальным значение и конечным)
// const END_TIMER_COUNTER = 1; // 1.6 секунды Конечное значение интервала для счетчика (счетчик убывает со случайным интервалом между начальным значение и конечным)
// const START_TIMER_COMMENTS = 1; // 3 секунды Начальное значение интервала для добавления комментов (комменты добавляются со случайным интервалом между начальным значение и конечным)
// const END_TIMER_COMMENTS = 1; // 25 секунд Конечное значение интервала для добавления комментов (комменты добавляются со случайным интервалом между начальным значение и конечным)
// const DELAY_ADD_COMMENTS = 1; // 5 секунд Задержка перед добавлением первого комментария
// const INTERVAL_CAROUSEL = 1; // 2 секунды Интервал перелистывания изображений
// const FADE_DURATION_MODAL = 1; // 0.2 секунды Длительность затухания модального окна (poopup)
// const INTERVAL_UPDATE_TIME_COMMETS = 1;
...
function addComment(params) {   
    ...
    $('#wcomments_posts').prepend(`        
            <div id="post164090831_34" class="_post post wcomments_post post--with-likes" data-post-id="164090831_34" post_view_hash="b10606087de5aa8396">
                <div class="_post_content">
                    <a target="_blank" class="post_image" href="${params.link}">
                        <img src="${params.avatar}" data-post-id="164090831_34" class="post_img">
                    </a>
                    <div class="post_content">
                        <div class="wcomments_post_content">                                                                       
                            <div class="post_author"><a target="_blank" class="author" href="${params.link}" data-from-id="164090831" data-post-id="164090831_34" data-post-click-type="post_owner_link">${params.name}</a><span class="explain"><span class="wall_fixed_label"> запись закреплена</span></span></div>
                            <div class="post_info">
                                <div class="wall_text">
                                    <div id="wpt164090831_34" class="wall_post_cont _wall_post_cont">
                                        <div class="wall_post_text zoom_text">${params.text}</div>
                                    </div>
                                </div>
                                <div class="wcomments_post_footer clear_fix">
                                    <div class="post_date"><span class="rel_date rel_date_needs_update" abs_time="${params.time}" time="${params.timer}">${params.time}</span></div>
                                    <div class="like_wrap _like_wall164090831_34  lite">
                                        <div class="like_cont ">
                                        </div>
                                    </div>
                                </div>
...
$('.cards__buy-btn').click(function () {
    $('.poopup__balance').text($(this).prev().prev().text());
    $('.poopup__price').text($(this).prev().text());
    $('.poopup__hidden-amount').val(($(this).prev().prev().text().replace(/\D+/g,"")));
